<html>
    <head>
	
       <meta charset="utf-8">
        <link rel="stylesheet" href="accTechnicien.css"  type="text/css" />
    </head>
    <body>
	
		<div id="entete">
			<h2 id="titre">Bienvenue sur la plateforme de commande de matériel</h2>

		
			<div id="cadreConnec">
			Nom d'utilisateur
			<p><a href="index.php">Se déconnecter</p></a>
			</div>
			
		
		</div>

		<div id="main">
			<div id="menu">
			<input type="submit" id='submit' value='DEMANDES EN ATTENTE' >
			<input type="submit" id='submit' value='DEMANDE TRAITEES' >
			<input type="submit" id='submit' value='CONTACTER UN ENSEIGNANT' >
			<input type="submit" id='submit' value='CONTACTER UN ELEVE' >
			<input type="submit" id='submit' value='RESULTATS ENQUETE' >
			
			</div>
			<div id="notifTchat">
			Notifications tchat
			</div>
			
			<div id="Carrousel">
			Carrousel d'images ?
			</div>
			

		<div id="contenu">
		Version Technicien
		<img src="images\logoPOPO.jpg" alt="" />
		</div>
		</div>

		<div id="footer">
		Site réalisé par Adrien Simard, Roshan Nepaul, Kévin Fanton et Yoann Raguenes
		<p> Etudiants en 3ème année de la filière IDU de Polytech Annecy-Chambéry</p>
			
		</div>
	</body>
</html>	